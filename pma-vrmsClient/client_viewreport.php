<?php
	...
	if($_SERVER['REQUEST_METHOD'] == 'POST'){
			...
			$query = "SELECT * FROM report WHERE plateNum LIKE '%" . $key . "%' OR owner LIKE '%" . $key . "%' OR datein LIKE '%" . $key . "%' OR timein LIKE '%" . $key . "%' OR violation LIKE '%" . $key . "%' OR  type LIKE '%" . $key . "%' ORDER BY rid DESC";
		...
		<div class='container'>
			<div class='row'>
				<div class='col-md-12' align="center">
					...
					<?php
									echo "<table align='center' id='viewtable'><tr><th class='head'>Plate No. </th><th class='head'>Registered Under</th><th class='head'>Date</th><th class='head'>Time</th><th class='head'>Violation</th><th class='head'>Type</th></tr>";
									...
									if(mysqli_num_rows($results) >= 1){
										while($row = mysqli_fetch_assoc($results)){
											...
											echo "<tr><td class='col'> " . $a . "</td><td class='col'>" . $b . "</td><td class='col'>" . $c . "</td><td class='col'>" . $d ."</td><td class='col'>" . $e ."</td><td class='col'>" . $f. "</td></tr>";
