<?php
	...
	if(isset($_POST)){
		...
		if ($_FILES['a_profile']['name'] == true){
			...
			$query1 = "UPDATE form2_militarypd SET m_lastname='$m_lastname', m_firstname='$m_firstname', m_middlename='$m_middlename', m_rank='$m_rank',
					m_brSvc='$m_brSvc', m_afpsn='$m_afpsn', m_residenceAddress='$m_residenceAddress', m_residenceTelNo='$m_residenceTelNo',
					m_emailAddress='$m_emailAddress', m_mobileNo='$m_mobileNo', m_designatedOffice='$m_designatedOffice', m_officeTelNo='$m_officeTelNo', 
					m_officeAddress='$m_officeAddress', 
					m_retirementDate='$m_retirementDate', m_class='$m_class', m_placeRegistered='$m_placeRegistered' WHERE m_militaryId='$id'";
		...
		if (isset($_POST['wheels'])) {
			...
			foreach ($wheels as $key => $wheel) {
				$query3 = "INSERT INTO vehicle_information (wheels, vehicleMake, plateNo, yearModel, color, motorNo, chassisNo, decalNo, driver_id, driver_type)
							VALUES('$wheel', '$vehicleMake[$key]', '$plateNo[$key]', '$yearModel[$key]', '$color[$key]', '$motorNo[$key]', '$chassisNo[$key]', '$decalNo[$key]', '$id', 'military')";
